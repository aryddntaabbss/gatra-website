...
export async function GET(req: NextRequest) {
  ...
  try {
    const data = await stravaFetch(`/clubs/${STRAVA_CONFIG.clubId}/activities?page=${page}&per_page=20`, token);
    return NextResponse.json({ status: "success", message: `${data.length} activities`, data });
  } catch {
